...
public class QueryDslGroupingTest {
    ...
    @Test
    @DisplayName("아이돌 그룹별로 아이돌의 그룹명과 평균 나이를 조회하세요. 평균 나이가 20세와 25세 사이인 그룹만 조회")
    void groupByGroupAndAvgAgeTest() {
        //given
        ...
        //then
        for (Tuple t : tuples) {
            String groupName = t.get(idol.group.groupName);
            Double avgAge = t.get(idol.age.avg());

            System.out.printf("그룹명: %s, 평균 나이: %.2f\n", groupName, avgAge);
        }
    }
